...
"""
                        ...
                        if max1>=self.th1.value() and xy[0]>0:
                            ...
                            if self.actionDrone.isChecked():
                                ...
                                if self.thonly.isChecked():
                                    self.dapper[0]=max1/self.th1.value()*100
                    ...
                    if xy[1] == max2:
                        ...
                        if max2>=self.th2.value() and xy[0]>0:
                            ...
                            if self.actionDrone.isChecked():
                                ...
                                if self.thonly.isChecked():
                                    self.dapper[1]=max2/self.th2.value()*100
                    ...
                    if xy[1] == max3:
                        ...
                        if max3 >= self.th3.value() and xy[0]>0:
                            ...
                            if self.actionDrone.isChecked():
                                ...
                                if self.thonly.isChecked():
                                    self.dapper[2]=max3/self.th3.value()*100
    ...
    def create(self):
        for i in range(0, 3):
            wid = Ui_MainWindow.CustomWidget("abcdefghijklmnopqrstu")  # 매개변수가 log 값이 되어야함

            wid2 = QtGui.QListWidgetItem()
            wid2.setSizeHint(QtCore.QSize(100, 60))
            self.list.addItem(wid2)
            self.list.setItemWidget(wid2, wid)
            self.list.setStyleSheet("QListWidget::item { border-bottom: 1px solid black; }")
